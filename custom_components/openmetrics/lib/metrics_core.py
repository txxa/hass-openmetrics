...
class UnknownMetricFamily(Metric):
    ...
    def add_metric(
        ...
        self.samples.append(
            Sample(
                self.name,
                dict(zip(self._labelnames, labels, strict=False)),
                value,
                timestamp,
            )
        )
...
class HistogramMetricFamily(Metric):
    ...
    def __init__(
        ...
        name: str,
        documentation: str,
        buckets: Sequence[tuple[str, float] | tuple[str, float, Exemplar]]
        | None = None,
        sum_value: float | None = None,
        labels: Sequence[str] | None = None,
        unit: str = "",
        ...
        for b in buckets:
            ...
            self.samples.append(
                Sample(
                    self.name + "_bucket",
                    dict(
                        list(zip(self._labelnames, labels, strict=False))
                        + [("le", bucket)]
                    ),
                    value,
                    timestamp,
...
class InfoMetricFamily(Metric):
    ...
    def add_metric(
        ...
        self.samples.append(
            Sample(
                self.name + "_info",
                dict(dict(zip(self._labelnames, labels, strict=False)), **value),
                1,
                timestamp,
            )
        )
